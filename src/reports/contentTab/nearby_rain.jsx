...
const Nearby_rain = ({ orgUnitId, year }) => {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  const [options, setOptions] = useState({});

  const url = `https://hfml.gov.la/hfml/api/29/analytics/events/query/gr24luudE0t.json?dimension=pe:${year}&dimension=ou:${orgUnitId}&dimension=rsXdExpMW65&dimension=Jy7ou2LCeju&dimension=WH4Az6TJ5ZA&dimension=f9d4P9maZEq&dimension=MLBhJz9GKds.WOFwxctknr3&dimension=MLBhJz9GKds.ttoaHSY8sTB&dimension=MLBhJz9GKds.XaW1WBFbzid&dimension=MLBhJz9GKds.j7JHlTSVlPy&dimension=MLBhJz9GKds.T08IOF1bwi3&dimension=MLBhJz9GKds.R6lD44QJfa4&dimension=MLBhJz9GKds.ex7pwRhyyAM&dimension=MLBhJz9GKds.BiSiYIv9EFW&dimension=MLBhJz9GKds.cC9tXe9nM1O&dimension=MLBhJz9GKds.VSbTF1Y4CMD&dimension=MLBhJz9GKds.thA1ltseEo4&dimension=MLBhJz9GKds.Lh7BukXsQES&dimension=MLBhJz9GKds.xcFzGlfAZNW&stage=MLBhJz9GKds&displayProperty=NAME&totalPages=false&outputType=EVENT&desc=eventdate&paging=false`;

  const equipmentConfig = [
    { key: "nearby", name: [28,24,35,30], bike: 33, walk: 31, boat: 23, km: 26 , feetruck:36, feehuman:32,feeboat:29,Dangerous:37,rain:22},
  ];
  ...
  if (!rows.length)
    return (
      <div className="container mt-3" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
        <h4 className="mb-3">4.2.2 ການເດີນທາງໄປ ສະຖານທີ່ບໍລິການໃກ້ຄຽງ ໃນລະດູຝົນ  </h4>
